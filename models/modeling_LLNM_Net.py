...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, rr=None, img_fea=None, sex=None, age=None):
        embedding_output, rr, img_fea, sex, age = self.embeddings(input_ids, rr, img_fea, sex, age)
        text = torch.cat((rr, img_fea, sex, age), 1)
...
class LLNM_Net(nn.Module):
    ...
    def forward(self, x, rr=None, img_fea=None, sex=None, age=None, labels=None):
        x, attn_weights = self.transformer(x, rr, img_fea, sex, age)
...
CONFIGS = {
    'LLNM_Net': configs.get_LLNM_Net_config(),
}
